# DNS Failover для Cloudflare

Автоматическая система переключения DNS записей в Cloudflare при недоступности серверов.

## Описание

Это приложение решает проблему недоступности сайта при падении одного из провайдеров интернета. Когда основной сервер становится недоступным, приложение автоматически переключает DNS запись в Cloudflare на резервный IP адрес.

## Возможности

- ✅ Мониторинг доступности серверов (TCP/HTTP проверки)
- ✅ Автоматическое переключение DNS записей через Cloudflare API
- ✅ Настраиваемые приоритеты серверов
- ✅ Подробное логирование всех операций
- ✅ Настраиваемые интервалы проверки и пороги недоступности
- ✅ Поддержка нескольких серверов
- ✅ Тестовый режим для проверки конфигурации
- ✅ Расширенная проверка DNS изменений через множественные DNS серверы

## Установка

1. **Установите Python 3.7 или новее**

2. **Установите зависимости:**
   ```bash
   pip install -r requirements.txt
   ```

3. **Настройте конфигурацию в `config.yaml`:**

   - **Cloudflare API Token:** Создайте токен в [Cloudflare Dashboard](https://dash.cloudflare.com/profile/api-tokens)
     - Перейдите в "My Profile" → "API Tokens" → "Create Token"
     - Используйте шаблон "Custom token"
     - Разрешения: Zone:Read, DNS:Edit
     - Ресурсы: Include - Specific zone - ваш домен

   - **Zone ID:** Найдите в обзоре вашего домена в Cloudflare Dashboard

   - **Настройте IP адреса ваших серверов**

## Конфигурация

Основные параметры в `config.yaml`:

```yaml
cloudflare:
  api_token: "ваш_api_токен_cloudflare"
  zone_id: "ваш_zone_id"
  domain_name: "ваш_домен.com"

servers:
  primary:
    ip: "192.168.1.100"    # IP первого провайдера
    port: 80
    priority: 1            # Наивысший приоритет

  secondary:
    ip: "192.168.1.101"    # IP второго провайдера  
    port: 80
    priority: 2

monitoring:
  check_interval: 60       # Интервал проверки в секундах
  timeout: 10             # Тайм-аут проверки
  failure_threshold: 3    # Количество неудач перед переключением
  check_method: "tcp"     # Метод проверки: "tcp" или "http"
```

## Использование

### Запуск сервиса
```bash
python main.py
```

### Тестирование конфигурации
```bash
python main.py --test
```

### Просмотр текущего статуса
```bash
python main.py --status
```

### Использование альтернативного файла конфигурации
```bash
python main.py --config /path/to/config.yaml
```

## Тестирование системы

### Базовое тестирование конфигурации

Для проверки правильности настройки системы используйте скрипт `test.py`:

```bash
python test.py
```

Этот скрипт выполняет комплексную проверку всех компонентов системы:

#### 🔧 Проверка конфигурации
- Валидация структуры конфигурационного файла
- Проверка наличия всех обязательных параметров
- Предупреждения о потенциальных проблемах

```
🔍 Проверка конфигурации...
✅ Конфигурация валидна
⚠️  Слишком частые проверки могут нагружать серверы
```

#### 🌐 Тестирование Cloudflare API
- Проверка подключения к Cloudflare API
- Валидация API токена и разрешений
- Получение текущей DNS записи

```
🔍 Тестирование подключения к Cloudflare API...
✅ Cloudflare API работает
   Текущая DNS запись: example.com -> 192.168.1.100
   TTL: 300 секунд
```

#### 🖥️ Проверка доступности серверов
- TCP подключение к каждому серверу
- HTTP проверки (если настроены)
- Отчет по каждому серверу

```
🔍 Тестирование подключения к серверам...

   Тестирование primary (192.168.1.100:80)...
   ✅ TCP соединение: OK

   Тестирование secondary (192.168.1.101:80)...
   ✅ TCP соединение: OK
```

#### 🔄 Тестирование DNS обновления
- Безопасный тест изменения DNS (dry run по умолчанию)
- Реальный тест с автоматическим восстановлением (с флагом `--real-test`)

```
🔍 Тестирование обновления DNS (DRY RUN)...
   Текущий IP: 192.168.1.100
   ✅ DNS обновление возможно (тестовый IP: 192.168.1.101)
   💡 Для реального тестирования запустите с параметром --real-test
```

### Режимы тестирования

#### Безопасное тестирование (по умолчанию)
```bash
python test.py
```
Проверяет все компоненты без изменения DNS записей.

#### Реальное тестирование DNS
```bash
python test.py --real-test
```
⚠️ **Внимание**: Временно изменяет DNS запись для проверки функциональности, затем автоматически восстанавливает.

#### Тестирование с альтернативной конфигурацией
```bash
python test.py --config другой_config.yaml
```

### Итоговый отчет

После завершения всех тестов вы получите подробный отчет:

```
==================================================
📊 ИТОГОВЫЙ ОТЧЕТ
==================================================
✅ Cloudflare API: OK
Серверы:
  ✅ primary: OK
  ✅ secondary: OK
  ❌ backup: FAILED
✅ DNS обновление: OK

🎉 ОБЩИЙ РЕЗУЛЬТАТ: ЕСТЬ ПРОБЛЕМЫ
❌ Исправьте найденные проблемы перед запуском сервиса
```

### Когда использовать test.py

- **Первоначальная настройка**: Проверить правильность конфигурации
- **Перед продакшеном**: Убедиться, что все работает корректно
- **Диагностика проблем**: Найти причину неполадок
- **Плановые проверки**: Регулярно тестировать работоспособность
- **После изменений**: Проверить систему после обновления конфигурации

### Устранение ошибок

#### ❌ "DNS запись не найдена"
```bash
python test.py --config config.yaml
```
Проверьте правильность `domain_name` и `zone_id` в конфигурации.

#### ❌ "Ошибка подключения к Cloudflare"
Проверьте:
- Правильность API токена
- Разрешения токена (Zone:Read, DNS:Edit)
- Подключение к интернету

#### ❌ "Серверы недоступны"
Проверьте:
- IP адреса серверов
- Порты и настройки файрвола
- Доступность серверов из текущей сети

## Проверка DNS изменений

Для детальной проверки работы DNS используйте скрипт `dns_verify.py`:

### Проверка текущего состояния DNS
```bash
python dns_verify.py --check-only
```

Эта команда покажет, какие IP адреса возвращают различные DNS серверы для вашего домена:
```
🔍 Проверка DNS для example.com...
   📍 Локальный DNS: 192.168.1.100
   🌐 Google DNS (8.8.8.8): 192.168.1.100
   🌐 Cloudflare DNS (1.1.1.1): 192.168.1.100
   🌐 OpenDNS (208.67.222.222): 192.168.1.100
   🌐 Quad9 (9.9.9.9): 192.168.1.100
   🔗 Google DNS API: 192.168.1.100
   🔗 Cloudflare DNS API: 192.168.1.100
```

### Полная проверка с реальным изменением DNS
```bash
python dns_verify.py
```

Этот тест:
1. ✅ Сохраняет текущую DNS запись
2. ✅ Изменяет DNS на альтернативный IP из конфигурации
3. ✅ Проверяет распространение изменений через множественные DNS серверы
4. ✅ Автоматически восстанавливает исходную DNS запись
5. ✅ Показывает детальный отчет о результатах

Пример вывода:
```
🚀 Запуск теста изменения DNS с верификацией
============================================================
🎯 Тестируемый домен: example.com
📍 Исходный IP: 192.168.1.100
🔄 Тестовый IP: 192.168.1.101 (сервер: secondary)

🔄 Изменение DNS записи на 192.168.1.101...
✅ DNS запись изменена в Cloudflare
✅ Cloudflare API подтверждает изменение: 192.168.1.101

⏳ Ожидание распространения DNS изменений...
📊 Правильный IP на 6/7 серверах
✅ DNS изменения распространились за 45 секунд!

🔄 Восстановление исходной DNS записи...
✅ DNS запись восстановлена

📊 ИТОГОВЫЙ ОТЧЕТ
============================================================
🎉 ТЕСТ УСПЕШЕН! DNS изменения работают корректно.
```

### Проверка конкретного домена
```bash
python dns_verify.py --check-only --domain example.com
```

### Использование альтернативной конфигурации
```bash
python dns_verify.py --config другой_config.yaml
```

### Что проверяет dns_verify.py

- **Множественные DNS серверы**: Google DNS, Cloudflare DNS, OpenDNS, Quad9
- **DNS API**: Google DNS API и Cloudflare DNS API
- **Локальный резолвер**: Системный DNS резолвер
- **Время распространения**: Отслеживает, как быстро изменения распространяются
- **Автовосстановление**: Гарантирует возврат к исходному состоянию

Этот инструмент особенно полезен для:
- Проверки правильности настройки DNS failover
- Определения времени распространения DNS изменений
- Диагностики проблем с DNS
- Тестирования перед продакшеном

## Логирование

Приложение ведет подробные логи:
- Консольный вывод для мониторинга в реальном времени
- Файл `dns_failover.log` для долгосрочного хранения
- Настраиваемый уровень логирования (DEBUG, INFO, WARNING, ERROR)

## Примеры сценариев работы

### Сценарий 1: Нормальная работа
1. Основной сервер (priority: 1) доступен
2. DNS запись указывает на основной IP
3. Периодические проверки подтверждают доступность

### Сценарий 2: Переключение на резерв
1. Основной сервер становится недоступным
2. После 3 неудачных проверок (failure_threshold)
3. DNS автоматически переключается на резервный IP
4. Логируется событие переключения

### Сценарий 3: Восстановление основного сервера
1. Основной сервер снова становится доступным
2. Система автоматически переключается обратно (по приоритету)
3. DNS возвращается на основной IP

## Безопасность

- API токен Cloudflare храните в безопасности
- Используйте минимальные необходимые разрешения для токена
- Регулярно проверяйте логи на предмет подозрительной активности

## Troubleshooting

### Проблема: "DNS запись не найдена"
- Убедитесь, что домен существует в указанной зоне Cloudflare
- Проверьте правильность zone_id и domain_name

### Проблема: "Ошибка авторизации API"
- Проверьте корректность API токена
- Убедитесь, что токен имеет необходимые разрешения

### Проблема: "Серверы недоступны"
- Проверьте настройки файрвола
- Убедитесь, что указанные порты открыты
- Попробуйте изменить check_method с "tcp" на "http"

### Проблема: "DNS изменения не распространяются"
- Используйте `python dns_verify.py` для диагностики
- Проверьте TTL настройки в конфигурации (рекомендуется 300 секунд или меньше)
- Убедитесь, что нет кеширования на уровне CDN или прокси

## Мониторинг производительности

Приложение выводит статус каждые 5 минут:
```
DNS Failover Status - 2024-10-02T15:30:00
============================================================
Активный сервер: primary

Статус серверов:
  ✅ primary [АКТИВНЫЙ]
     IP: 192.168.1.100:80
     Приоритет: 1
     Ошибок подряд: 0

  ✅ secondary
     IP: 192.168.1.101:80
     Приоритет: 2
     Ошибок подряд: 0
============================================================
```

## Автозапуск

### Windows (Task Scheduler)
1. Откройте "Планировщик заданий"
2. Создайте новое задание
3. Укажите команду: `python C:\path\to\main.py`
4. Настройте запуск при старте системы

### Linux (systemd)
Создайте файл `/etc/systemd/system/dns-failover.service`:
```ini
[Unit]
Description=DNS Failover Service
After=network.target

[Service]
Type=simple
User=your-user
WorkingDirectory=/path/to/dns-api
ExecStart=/usr/bin/python3 /path/to/dns-api/main.py
Restart=always

[Install]
WantedBy=multi-user.target
```

Активируйте сервис:
```bash
sudo systemctl enable dns-failover.service
sudo systemctl start dns-failover.service
```